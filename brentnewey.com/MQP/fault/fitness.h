...
int fitness(struct site a_site) {
	...
	task[0] = 0;	//amphibians (dfrog is first on page - could be 5)
	task[1] = 1;	//mammals (ardolphin first on page - could be 6)
	task[2] = 5;	//dfrog - also 14
	task[3] = 7;	//dmongoose - 16
	task[4] = 8;	//tdevil - 17
	task[5] = 10;	//bmaster - 19
	task[6] = 11;	//kdragon - 20

	//error rates for the tasks
	...
	//task fitness = wordcount + Weighted(error*pagecount)
	int weight=100;
	for (int i=0; i < 7; i++)
	{
		sumfit += wordcount(temp_site, task[i]) + (weight*(pagecount(temp_site, task[i]))*error[i]);
	}
